...
def get_price(ticker):
    ...
    try:
        ...
        print('Error, please try another ticer')
...
layout = html.Div(children=[
    ...
    html.Div(children='Dashboard - Quarterly financial statements', className='ml-4',
...
def roae(dat):
    ...
    test1 = go.Scatter(y=chart.select(pl.col("roe_4Q")).to_series(), x=date, name="ROE", yaxis='y2',
                       line=dict(color='red', width=3), mode='lines',line_shape='spline')
    test2 = go.Scatter(y=chart.select(pl.col("roe_core_4Q")).to_series(), x=date, name="Core ROE", yaxis='y2',
...
def asset(dat):
    ...
    asset_bar9 = go.Scatter(y=chart.select(pl.col("marketCap")).to_series(), x=date, yaxis='y', name="Market Capitilization",
...
def equity(dat):
    ...
    asset_bar10 = go.Scatter(y=chart.select(pl.col("marketCap")).to_series(), x=date, name="Market Capitilization",
                             line=dict(color='darkturquoise', width=3), mode='lines+markers')
...
def growth(dat):
    ...
    fig = go.Figure(data=g, layout=go.Layout(barmode='relative'
                                                      , xaxis=dict(tickformat='%Y-%b', showgrid=False)
                                                      , yaxis=go.layout.YAxis(showgrid=True, tickformat='.1%',
                                                                               overlaying='y'
                                                                               )
...
def pe(dat):
    chart = pl.from_pandas(pd.read_json(dat))
    date = chart.select(pl.col("dates")).to_series()
    bar1 = go.Scatter(y=chart.select(pl.col("Lãi/(lỗ) thuần sau thuế_4Q")).to_series(), x=date, name='Net profit 4Q (left)',
                      line=dict(color='red', width=3,dash='dot'), mode='lines',line_shape='spline')
    bar2 = go.Scatter(y=chart.select(pl.col("core_e_4Q")).to_series(), x=date, name='Core Net profit 4Q (left)',
    ...
    bar4 = go.Scatter(y=chart.select(pl.col("P/E")).to_series(), x=date, name="P/E (right)",
...
def pb(dat):
    ...
    bar2 = go.Scatter(y=chart.select(pl.col("marketCap")).to_series(), x=date, name='Market capitalization (left)',
                      line=dict(color='darkturquoise', width=3), mode='lines+markers')
    bar3 = go.Scatter(y=chart.select(pl.col("roe_4Q")).to_series(), x=date, name="ROE (right)",
                      line=dict(color='red', width=3,dash='dot'), mode='lines',line_shape='spline', yaxis='y2')
    bar4 = go.Scatter(y=chart.select(pl.col("roe_core_4Q")).to_series(), x=date, name="ROE_core (right)",
